...
module DatadogAPIClient::V2
  class UsageMeteringAPI
    ...
    #
    # @see #get_custom_events_count_with_http_info
    def get_custom_events_count(start_hr, opts = {})
      data, _status_code, _headers = get_custom_events_count_with_http_info(start_hr, opts)
      data
    end
    ...
    # @return [Array<(UsageCustomEventsResponse, Integer, Hash)>] UsageCustomEventsResponse data, response status code and response headers
    def get_custom_events_count_with_http_info(start_hr, opts = {})

      if @api_client.config.unstable_operations.has_key?(:get_custom_events_count)
        unstable_enabled = @api_client.config.unstable_operations[:get_custom_events_count]
        if unstable_enabled
          @api_client.config.logger.warn format("Using unstable operation '%s'", "get_custom_events_count")
        else
          raise APIError.new(message: format("Unstable operation '%s' is disabled", "get_custom_events_count"))
        end
      end

      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: UsageMeteringAPI.get_custom_events_count ...'
      end
      # verify the required parameter 'start_hr' is set
      if @api_client.config.client_side_validation && start_hr.nil?
        fail ArgumentError, "Missing the required parameter 'start_hr' when calling UsageMeteringAPI.get_custom_events_count"
      end
      # resource path
      local_var_path = '/api/v2/usage/custom_events'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'UsageCustomEventsResponse'

      # auth_names
      auth_names = opts[:debug_auth_names] || [:apiKeyAuth, :appKeyAuth, :AuthZ]

      new_options = opts.merge(
        :operation => :get_custom_events_count,
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(Net::HTTP::Get, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: UsageMeteringAPI#get_custom_events_count\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
