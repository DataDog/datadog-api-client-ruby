...
module DatadogAPIClient::V2
  class LLMObservabilityAPI
    ...
    def list_llm_obs_custom_eval_configs(opts = {})
      data, _status_code, _headers = list_llm_obs_custom_eval_configs_with_http_info(opts)
      data
    end
    ...
    def list_llm_obs_custom_eval_configs_with_http_info(opts = {})
      unstable_enabled = @api_client.config.unstable_operations["v2.list_llm_obs_custom_eval_configs".to_sym]
      if unstable_enabled
        @api_client.config.logger.warn format("Using unstable operation '%s'", "v2.list_llm_obs_custom_eval_configs")
      else
        raise DatadogAPIClient::APIError.new(message: format("Unstable operation '%s' is disabled", "v2.list_llm_obs_custom_eval_configs"))
      end

      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: LLMObservabilityAPI.list_llm_obs_custom_eval_configs ...'
      end
      # resource path
      ...
      data, status_code, headers = @api_client.call_api(Net::HTTP::Get, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: LLMObservabilityAPI#list_llm_obs_custom_eval_configs\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
