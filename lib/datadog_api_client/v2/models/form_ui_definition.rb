  ...
  class FormUiDefinition
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DatadogAPIClient::V2::FormUiDefinition` initialize method"
      end
      ...
      if attributes.key?(:'ui_order')
        if (value = attributes[:'ui_order']).is_a?(Array)
          self.ui_order = value
        end
      end
    ...
    # @!visibility private
    def hash
      [ui_order, ui_theme, additional_properties].hash
    end
  end
end
