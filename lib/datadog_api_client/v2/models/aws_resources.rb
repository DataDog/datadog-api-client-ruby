...
module DatadogAPIClient::V2
  ...
  class AWSResources
    ...
    def self.openapi_types
      {
        :'cloud_security_posture_management_collection' => :'Boolean',
        :'extended_collection' => :'Boolean'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DatadogAPIClient::V2::AWSResources` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DatadogAPIClient::V2::AWSResources`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'cloud_security_posture_management_collection')
        self.cloud_security_posture_management_collection = attributes[:'cloud_security_posture_management_collection']
      end

      if attributes.key?(:'extended_collection')
        self.extended_collection = attributes[:'extended_collection']
      end
    end
