  ...
  class SyntheticsNetworkAssertionPacketLossPercentage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DatadogAPIClient::V2::SyntheticsNetworkAssertionPacketLossPercentage` initialize method"
      end
    ...
    def target=(target)
      ...
      if target > 1
        fail ArgumentError, 'invalid value for "target", must be smaller than or equal to 1.'
      end
      if target < 0
        fail ArgumentError, 'invalid value for "target", must be greater than or equal to 0.'
      end
      @target = target
    end
    ...
    # @!visibility private
    def hash
      [operator, target, type, additional_properties].hash
    end
  end
end
