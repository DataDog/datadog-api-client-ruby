  ...
  class CycloneDXTools
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DatadogAPIClient::V2::CycloneDXTools` initialize method"
      end
    ...
    def components=(components)
      ...
      if components.length > 1
        fail ArgumentError, 'invalid value for "components", number of items must be less than or equal to 1.'
      end
      if components.length < 1
    ...
    # @!visibility private
    def hash
      [components, additional_properties].hash
    end
  end
end
