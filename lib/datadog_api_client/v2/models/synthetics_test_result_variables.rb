...
module DatadogAPIClient::V2
  ...
  class SyntheticsTestResultVariables
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DatadogAPIClient::V2::SyntheticsTestResultVariables` initialize method"
      end
      ...
      if attributes.key?(:'extracted')
        if (value = attributes[:'extracted']).is_a?(Array)
    ...
    # @!visibility private
    def hash
      [config, extracted, additional_properties].hash
    end
  end
end
