  ...
  class PointPlotProjection
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DatadogAPIClient::V1::PointPlotProjection` initialize method"
      end
      ...
      if attributes.key?(:'extra_columns')
        if (value = attributes[:'extra_columns']).is_a?(Array)
          self.extra_columns = value
        end
      end
    ...
    # @!visibility private
    def hash
      [dimensions, extra_columns, type, additional_properties].hash
    end
  end
end
