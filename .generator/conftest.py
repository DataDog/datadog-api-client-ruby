...
RUBY_EXAMPLE_J2 = JINJA_ENV.get_template("example.j2")
...
def pytest_bdd_after_scenario(request, feature, scenario):
    ...
    output = ROOT_PATH / "examples" / version / group_name / f"{operation_id}{unique_suffix}.rb"
...
def request_body_from_file(request, context, path, api_version):
    """Set request body."""
    body_file = ROOT_PATH / "features" / api_version / path
...
def build_given(version, operation):
    def wrapper(request, context, specs, operation_specs):
        def make_path(keys):
...
def expect_response_items(context, fixture_length):
    """Check the length of a response."""
