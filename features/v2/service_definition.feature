...
Feature: Service Definition
  ...
  Scenario: Delete a single service definition returns "OK" response
    Given there is a valid "service_definition" in the system
    And new "DeleteServiceDefinition" request
    And request contains "service_name" parameter with value "service-{{ unique }}"
    When the request is sent
  ...
  Scenario: Get a single service definition returns "OK" response
    Given there is a valid "service_definition" in the system
    And new "GetServiceDefinition" request
    And request contains "service_name" parameter with value "service-{{ unique }}"
    When the request is sent
