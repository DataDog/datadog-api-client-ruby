...
Feature: Teams
  ...
  @generated @team:DataDog/aaa-omg
  Scenario: Add a user to a team returns "Represents a user's association to a team" response
    ...
    And body with value {"data": {"attributes": {"role": "admin"}, "relationships": {"team": {"data": {"id": "{{ dd_team.data.id }}", "type": "team"}}, "user": {"data": {"id": "{{ user.data.id }}", "type": "users"}}}, "type": "team_memberships"}}
    When the request is sent
